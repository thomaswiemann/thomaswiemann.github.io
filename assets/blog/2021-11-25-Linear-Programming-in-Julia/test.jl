...
# Specify custom functions to compute gammas and bounds via LP

# function to get gammas
function get_gamma(X, w1, w0)
    Xw1 = X .* mapslices(mean, w1, dims=2)
    Xw0 = X .* mapslices(mean, w0, dims=2)
    return mapslices(mean, Xw1, dims=1), mapslices(mean, Xw0, dims=1)
end
...
b_ATT = mean(w1_ATT .* m1) + mean(w0_ATT .* m0); # implied ATT

# Print \beta_s
print((b_ATT = b_ATT, b_IV=b_IV, b_TSLS=b_TSLS))
...
# ATT 
p=plot(_x,ones(19)*b_ATT, line = (:line, :dot, 0.8, 1, :black), label="")
